...
$status_ins = false;
$status_act = false;
$btn_text = esc_html__('Install All', 'autoev');
...
$merlin_setup = get_option( 'merlin_' . autoev()->get_slug() . '_completed' );
 
if( $status_ins && $status_act)
	$btn_text = esc_html__('Install & Active All', 'autoev');
else if($status_ins && !$status_act)
	$btn_text = esc_html__('Install All', 'autoev');
else if(!$status_ins && $status_act)
	$btn_text = esc_html__('Active All', 'autoev');
?>
<main>
	...
	<div class="pxl-dashboard-wrap">
		...
		<header class="pxl-dsb-header admin-plugin">
			<div class="pxl-dsb-header-inner">
				<h4><?php esc_html_e( 'Install Plugins', 'autoev' ); ?></h4>
			...
			<p><?php esc_html_e( 'Make sure to activate required plugins prior to import a demo.', 'autoev' ); ?></p> 
		</header>
	        ...
	        <?php
				...
				foreach( $plugins as $plugin ) :
					...
					elseif( !is_plugin_inactive( $file_path ) ) {
						...
						$display_status = esc_html__( 'Active:', 'autoev' );
					}
			?>
				<div class="pxl-col pxl-col-3">
					...
					<?php 
					$barplugin = new Autoev_Admin_Plugins;
