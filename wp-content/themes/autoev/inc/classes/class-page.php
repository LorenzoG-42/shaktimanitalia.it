...
if (!class_exists('Autoev_Page')) {

    class Autoev_Page
    {
        public function get_site_loader(){

            $site_loader = autoev()->get_theme_opt( 'site_loader', false );
            if($site_loader) { ?>
        ...
        public function get_page_title(){
            ...
            $pt_mode = autoev()->get_opt('pt_mode');
            $pt_mode_product = autoev()->get_opt('pt_mode_product');
            $ptitle_scroll_opacity = autoev()->get_opt('ptitle_scroll_opacity');
            $ptitle_layout = (int)autoev()->get_opt('ptitle_layout');
            $ptitle_layout_product = (int)autoev()->get_opt('ptitle_layout_product');
            ...
            <?php } elseif($pt_mode == 'df') {
                $ptitle_breadcrumb_on = autoev()->get_opt( 'ptitle_breadcrumb_on', '1' ); 
                    ...
                    <div class="container">
                        <div class="row">
                            <div class="col-sm-12 col-md-6 col-lg-6">
                                <h1 class="pxl-page-title"><?php echo autoev_html($titles['title']) ?></h1>
                            </div>
        ...
        public function get_title() {
            ...
            if ( ! is_archive() ) {
                ...
                elseif ( is_page() ) {
                    ...
                    $title = esc_html__( '404 Error', 'autoev' );
                } elseif ( is_search() ) {
                    $title = esc_html__( 'Search results', 'autoev' );
                } elseif ( is_singular('lp_course') ) {
                    $title = esc_html__( 'Course', 'autoev' );
                } else {
                    $title = get_post_meta( get_the_ID(), 'custom_title', true );
                    if ( ! $title ) {
                        $title = get_the_title();
        ...
        public function get_breadcrumb(){
            ...
            foreach ( $entries as $entry )
            {
                ...
                {
                    $sg_post_title = autoev()->get_theme_opt('sg_post_title', 'default');
                    $sg_post_title_text = autoev()->get_theme_opt('sg_post_title_text');
                    if(is_singular('post') && $sg_post_title == 'custom_text' && !empty($sg_post_title_text)) {
                        $entry_label = $sg_post_title_text;
                    }
                    $sg_product_ptitle = autoev()->get_theme_opt('sg_product_ptitle', 'default');
                    $sg_product_ptitle_text = autoev()->get_theme_opt('sg_product_ptitle_text');
        ...
        public function get_pagination( $query = null, $ajax = false ){
            ...
            if($ajax){
                add_filter('paginate_links', 'autoev_ajax_paginate_links');
            }
