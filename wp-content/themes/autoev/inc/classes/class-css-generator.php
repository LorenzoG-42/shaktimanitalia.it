...
class Autoev_CSS_Generator {
	...
	function __construct() {
		$this->opt_name = autoev()->get_option_name();  
        ...
        add_action( 'wp_enqueue_scripts', array( $this, 'autoev_enqueue' ), 20 );
	}
	...
	function init() {
		...
		add_action( 'wp', array( $this, 'autoev_generate_with_dev_mode' ) );
		add_action( "redux/options/{$this->opt_name}/saved", function () {
			$this->autoev_generate_file_options();
		} );
	}

	function autoev_generate_with_dev_mode() {
		if ( $this->dev_mode === true ) {
            $this->autoev_generate_file_options();
			$this->autoev_generate_file();
		}
	}

    function autoev_generate_file_options() {
        ...
        $this->redux->filesystem->execute( 'put_contents', $_options, array(
            'content' => preg_replace( "/(?<=[^\r]|^)\n/", "\r\n", $this->autoev_options_output() )
        ) );
    }

	function autoev_generate_file() {
		$scss_dir = get_template_directory() . '/assets/scss/';
	...
	protected function autoev_options_output() {
		$theme_colors                    = autoev_configs('theme_colors');
        $links                           = autoev_configs('link');
        $gradients                       = autoev_configs('gradient');
		ob_start();

		printf('$autoev_theme_colors_key:(%s);',$this->print_scss_opt_colors($theme_colors,'key'));
        printf('$autoev_theme_colors_val:(%s);',$this->print_scss_opt_colors($theme_colors,'val'));
        // color rgb only
    ...
    /* Inline CSS */
    function autoev_enqueue() {
        $css = $this->autoev_inline_css();
        if ( !empty( $css ) ) {
            wp_add_inline_style( 'pxl-style', $css );
        }
    }
    protected function autoev_inline_css() {
        ob_start();
